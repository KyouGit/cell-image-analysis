...
OUT_BASE  = "/home/smile/work/hdd8t/EXP-005c_EigenCAM_v4"
...
N_SAMPLES   = 3
N_RBC_PAIRS = 5   # number of RBC pairs
IMG_SIZE    = 640
PANEL_W     = 280
PANEL_H     = 280
PAD_FACTOR  = 2.5
CROP_MARGIN = 4   # pixels inside boundary required
...
def is_crop_clean(img_path, cx, cy, bw, bh):
    ...
    if crop.min() < 12:       # slide boundary / dark artifact
        return False
    if crop.mean() > 230:     # near-blank (white background)
        return False
    if crop.std() < 8:        # uniform / featureless
        return False
    return True
...
# Pre-pool enough RBC samples for all 5 RBC pairs (each needs N_SAMPLES)
...
for cls_a, cls_b, pair_name, category, exclude in PAIRS:
    ...
    # For RBC (cls_b==12), use pre-pooled samples to avoid duplicate images
    if cls_b == 12:
