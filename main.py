...
class ConvAutoEncoder(nn.Module):
    """
    Convolutional AutoEncoder for image feature extraction
    
    Architecture:
        Encoder: Conv layers → Bottleneck (latent representation)
...
def visualize_latent_space(features, labels, class_names, method='umap', save_path='latent_space.png'):
    """
...
def main():
    ...
    # Configuration
    DATA_DIR = 'data/cells'  # Change to your data directory
    LATENT_DIM = 128
    BATCH_SIZE = 32
    EPOCHS = 50
    IMG_SIZE = 128
    ...
    dataset = CellImageDataset(DATA_DIR, img_size=IMG_SIZE)
    ...
    visualize_latent_space(features, labels, dataset.idx_to_class, 
                          method='umap', save_path='umap_visualization.png')
    
    # 7. Visualize with t-SNE
    print("\n[Step 7] Visualizing latent space with t-SNE...")
    visualize_latent_space(features, labels, dataset.idx_to_class,
                          method='tsne', save_path='tsne_visualization.png')
    
    # 8. Analyze class overlap
    print("\n[Step 8] Analyzing class overlap...")
    analyze_class_overlap(features, labels, dataset.idx_to_class)
